...
describe('InstanceMethods', () => {
    describe('Sync vs Async', () => {
        it('When resolved with value, should return SyncPromise', () => {
            const promise = new SyncPromise_1.SyncPromise((resolve, reject) => {
                resolve(5);
            }).then(result => result, err => err);
            chai_1.expect(promise).instanceof(SyncPromise_1.SyncPromise);
        });
        it('When resolved with SyncPromise, should return SyncPromise', () => {
            const promise = new SyncPromise_1.SyncPromise((resolve, reject) => {
                resolve(SyncPromise_1.SyncPromise.resolve(5));
            }).then(result => result, err => err);
            chai_1.expect(promise).instanceof(SyncPromise_1.SyncPromise);
        });
        it('When resolved with Promise, should return Promise', () => {
            ...
            chai_1.expect(promise).instanceof(Promise);
        });
        ...
        it('When is rejected with SyncPromise, should return SyncPromise', () => {
            const promise = new SyncPromise_1.SyncPromise((resolve, reject) => {
                reject(SyncPromise_1.SyncPromise.resolve(5));
            }).then(result => result, err => err);
            chai_1.expect(promise).instanceof(SyncPromise_1.SyncPromise);
        });
    ...
    describe('.then', () => {
        ...
        describe('by resolved', () => {
            it('When is resolved with value, should pass that value to next "onfulfilled"', done => {
                new SyncPromise_1.SyncPromise((resolve, reject) => {
                    resolve(5);
                })
            ...
            it('When is resolved with rejected SyncPromise, should pass value of that SyncPromise to next "onrejected"', done => {
                new SyncPromise_1.SyncPromise((resolve, reject) => {
                    resolve(SyncPromise_1.SyncPromise.reject(5));
                })
                    .then(notCallableSpy, spy)
                    .then(() => {
                    chai_1.expect(spy.calledWith(5)).to.be.true;
        ...
        describe('by rejected', () => {
            ...
            it('When is rejected with resolved SyncPromise, should pass that SyncPromise to next "onrejected"', done => {
                const resolvedSyncPromise = SyncPromise_1.SyncPromise.resolve(5);
                new SyncPromise_1.SyncPromise((resolve, reject) => {
                    reject(resolvedSyncPromise);
                })
                    .then(notCallableSpy, spy)
                    .then(() => {
                    chai_1.expect(spy.calledWith(resolvedSyncPromise)).to.be.true;
                    chai_1.expect(notCallableSpy.notCalled).to.be.true;
                })
                    .then(() => done());
            });
            it('When is rejected with rejected SyncPromise, should pass that SyncPromise to next "onrejected"', done => {
                const spy = sinon.spy((err) => err.catch(err => err));
                const rejectedSyncPromise = SyncPromise_1.SyncPromise.reject(5);
                new SyncPromise_1.SyncPromise((resolve, reject) => {
                    reject(rejectedSyncPromise);
                })
                    .then(notCallableSpy, spy)
                    .then(() => {
                    chai_1.expect(spy.calledWith(rejectedSyncPromise)).to.be.true;
                    chai_1.expect(notCallableSpy.notCalled).to.be.true;
                })
                    .then(() => done());
            });
        ...
        describe('by throws exception', () => {
            it('When executor throws value, should pass that value to next "onrejected"', done => {
                new SyncPromise_1.SyncPromise((resolve, reject) => {
                    ...
                    .then(notCallableSpy, spy)
                    .then(result => {
                    chai_1.expect(spy.calledWith('FakeError')).to.be.true;
                    chai_1.expect(notCallableSpy.notCalled).to.be.true;
                })
                    .then(() => done());
            });
        ...
        describe('by chain', () => {
            ...
            let notCallableSpy;
            beforeEach(() => {
                spy1 = sinon.spy((res) => res);
                spy2 = sinon.spy((res) => res);
                notCallableSpy = sinon.spy();
            });
            it('When "onfulfilled" returns a value, should pass that value to next "onfulfilled"', done => {
                new SyncPromise_1.SyncPromise((resolve, reject) => {
                    ...
                    .then(() => {
                    chai_1.expect(spy1.calledWith(5)).to.be.true;
                    chai_1.expect(spy2.calledWith(5)).to.be.true;
                    chai_1.expect(notCallableSpy.notCalled).to.be.true;
                })
                    .then(() => done());
            });
            ...
            it('When "onfulfilled" throws a value, should pass that value to next "onrejected"', done => {
                new SyncPromise_1.SyncPromise((resolve, reject) => {
                    ...
                    .then(() => {
                    chai_1.expect(spy1.calledWith('FakeMessage')).to.be.true;
                    chai_1.expect(notCallableSpy.notCalled).to.be.true;
                })
                    .then(() => done());
            });
